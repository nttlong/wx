...
func TestDepens(t *testing.T) {
	...
	type Test struct {
		Fx wx.Depend[Test, Test]
	}
	type Test2 struct {
		Fx wx.Depend[Test, Test2]
	}
	ok := wx.Helper.IsGenericDepen(reflect.TypeOf(wx.Depend[Test, Test]{}))
	assert.True(t, ok)
	ok = wx.Helper.IsGenericDepen(reflect.TypeOf(&wx.Depend[Test, Test]{}))
	assert.True(t, ok)
	...
	assert.False(t, ok)
	fieldIndex := wx.Helper.GetDepen(reflect.TypeOf(wx.Depend[Test, Test]{}))
...
func BenchmarkTestDepens(t *testing.B) {
	...
	for i := 0; i < t.N; i++ {
		...
		wx.Helper.GetDepen(reflect.TypeOf(wx.Depend[Test, Test]{}))
